...
# temp paths (cross-platform: was hard-coded /tmp, broke on Windows)
import tempfile
_TMP = tempfile.gettempdir()
_TS_PATH = os.path.join(_TMP, "moves.ts")
_JS_PATH = os.path.join(_TMP, "moves.js")
_CJS_PATH = os.path.join(_TMP, "moves.cjs")
...
p = subprocess.Popen(
    ["node", _CJS_PATH], stdout=subprocess.PIPE, stderr=subprocess.PIPE
)
stdout = p.stdout.read()
stderr = p.stderr.read()
