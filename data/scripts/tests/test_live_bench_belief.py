...
from belief_state import BeliefState, _DEFAULT_PIKALYTICS_PATH  # noqa: E402
...
def test_serve_belief_level_matches_training_default():
    ...
    from belief_state import DEFAULT_LEVEL
...
def test_reenrich_reproduces_training_opp_bytes(belief):
    """The strongest guarantee: stripping then re-enriching the opponent of a
    real, belief-enriched TRAINING snapshot reproduces the SAME opponent bytes —
    i.e. the live gap-#6 splice matches the data the net was trained on."""
    import glob, json
    root = Path(__file__).resolve().parents[3]
    files = sorted(glob.glob(
        str(root / "data/vods/Prepared_training_data/Regulation_MA/Jsonl_TypeB/**/*.jsonl"),
        recursive=True))
    if not files:
        pytest.skip("no Type B training data present")

    enc = VodStateEncoder()
    live = LiveStateEncoder(belief=belief)
        ...
        with open(fp, encoding="utf-8") as fh:
            for line in fh:
                ...
                assert np.allclose(vec_train[opp_act], vec_serve[opp_act], atol=1e-6), \
                    "opp ACTIVE bytes diverge from training after re-enrichment"
                assert np.allclose(vec_train[opp_ben], vec_serve[opp_ben], atol=1e-6), \
                    "opp BENCH bytes diverge from training after re-enrichment"
                checked += 1
                if checked >= 60:
                    break
        if checked >= 60:
