...
from self_play.schema import PASS_ACTION  # noqa: E402
from self_play import game_runner as G  # noqa: E402
...
def test_phase0_report_pass():
    rep = G.phase0_report(_games([True, False] * 3), {"model": 600}, min_games=4)
    assert rep["PASS"]
    assert rep["model_driven"] == 1.0 and rep["symmetry_ok"]
...
def test_phase0_flags_low_model_driven():
    rep = G.phase0_report(_games([True, False] * 3),
                          {"model": 500, "retry_default": 100}, min_games=4)
    assert rep["model_driven"] == pytest.approx(500 / 600)
...
def test_print_phase0_report_runs(capsys):
    G.print_phase0_report(G.phase0_report(_games([True, False] * 3), {"model": 600}, min_games=4))
    out = capsys.readouterr().out
    assert "Phase-0" in out and "MODEL-DRIVEN" in out and "PASS" in out
