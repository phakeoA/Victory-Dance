...
def make(u, team):
    rp = _REPO/"replay_buffer"/f"_smoke_{u}.jsonl"
    if rp.exists():
        rp.unlink()
    return SmokePlayer(
...
async def main(n):
    ...
    ok1, bad1 = _states_finite(_REPO/"replay_buffer"/"_smoke_ZoroRed.jsonl")
    ok2, bad2 = _states_finite(_REPO/"replay_buffer"/"_smoke_ZoroBlue.jsonl")
