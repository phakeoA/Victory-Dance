...
for _p in (str(_REPO_ROOT / "data" / "scripts"), str(_REPO_ROOT),
           str(_REPO_ROOT / "local_battle")):
    if _p in sys.path:
        sys.path.remove(_p)
...
from self_play import policy_eval
from self_play.collector import TrajectoryCollector, assert_zero_sum
from self_play.reward import place_terminal_reward
from self_play.schema import PASS_ACTION, Transition, Trajectory
from self_play.store import assert_terminal_rewards_clean, write_trajectories
...
from player import VGCPlayer  # noqa: E402  (local spliced player)
...
class SelfPlayVGCPlayer(VGCPlayer):
    ...
    def _record_rl_decision(self, battle, state_vec, a0, a1, g0, g1, source, decision_type):
        ...
        try:
            ...
            record_decision(self._collector_for(battle), self._ac, state=state_vec,
                            a0=a0, a1=a1, g0=g0, g1=g1, mask0=m0, mask1=m1,
                            gmask0=gm0, gmask1=gm1, decision_type=decision_type,
                            turn=battle.turn, tau=self._tau)
        except Exception:
...
def phase0_report(pairs: List[tuple], source_counts: dict, *, min_games: int = 200,
    ...
    games: enough games, MODEL-DRIVEN >= 99%, zero-sum symmetry never trips, p1/p2
    win-rate ~50%, terminal-reward space clean, fallback rate ~0."""
    n = len(pairs)
    sym_fail = 0
    p1_wins = p1_games = 0
    for a, b in pairs:
    ...
    actions = {k: v for k, v in source_counts.items() if not str(k).startswith("tp_")}
    total = sum(actions.values())
    model = sum(actions.get(s, 0) for s in MODEL_DRIVEN_SOURCES)
    md = (model / total) if total else 0.0
    ...
    rep = {
        ...
        "model_driven": md, "model_driven_ok": md_ok, "fallback_rate": 1.0 - md,
        "total_decisions": total, "by_source": dict(actions),
    ...
    rep["PASS"] = bool(enough and md_ok and sym_ok and bal_ok and clean)
    return rep
...
def print_phase0_report(rep: dict) -> None:
    ...
    md = rep["model_driven"]
    print(f"  MODEL-DRIVEN      : {md*100:.2f}%  of {rep['total_decisions']} decisions  "
          f"(need >= 99%)  [{mark(rep['model_driven_ok'])}]")
    if md < 0.999:
        for k, v in sorted(rep["by_source"].items(), key=lambda kv: -kv[1]):
            if k not in MODEL_DRIVEN_SOURCES:
                print(f"      fallback {k:20s}: {v}")
...
async def run_self_play_games(
    ...
    from gauntlet import team_matchups
    ...
    try:
        for team_a, team_b, n in team_matchups(team_pool, n_games, seed=matchup_seed):
            ...
            p1 = SelfPlayVGCPlayer(
                ...
                replay_path=_REPO_ROOT / "replay_buffer" / f"SP1_{uid}.jsonl",
            ...
            p2 = SelfPlayVGCPlayer(
                ...
                replay_path=_REPO_ROOT / "replay_buffer" / f"SP2_{uid}.jsonl",
...
def main(argv=None) -> int:
    ...
    ap.add_argument("--store", default=str(_REPO_ROOT / "replay_buffer" / "selfplay_phase0.jsonl"))
    ...
    if not ckpt.exists():
        ...
        return 2
    from self_play.actor_critic import ActorCritic
    ac = ActorCritic.from_bc_checkpoint(ckpt)

    pairs, sources = asyncio.run(run_self_play_games(
        ac, team_pool=args.teams, n_games=args.games, store_path=Path(args.store),
