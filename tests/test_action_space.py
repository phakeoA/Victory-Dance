...
def _mon(species="Sneasler", moves=("Fake Out", "Close Combat", "Dire Claw", "Protect"), **over):
    d = {
        ...
        "revealed_moves": [],
        # v11 exact-mask: Fake Out is only legal on the first turn out, which is when a corpus snapshot
        # would have it selectable — so the bucket-mechanics fixtures mark first_turn (else the exact mask
        # correctly drops Fake Out and these target-bucket assertions test an unreachable state).
        "volatiles": {"first_turn": True},
    }
    d.update(over)
    return d
...
def vod_transitions(vod_path):
    from v_dance.parser.vod_parser import replay_to_transitions
