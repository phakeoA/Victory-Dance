...
# ══════════════════════════════════════════════════════════════════════════════
# CHOICE-LOCK FIX — a usable (Showdown-vouched) move is never dropped for a stale
# current_pp / cosmetic id mismatch, so a Choice-locked mon stays MODEL-DRIVEN.
# ══════════════════════════════════════════════════════════════════════════════
class _FakeMove:
    def __init__(self, mid, pp=16):
        self.id = mid
        self.current_pp = pp


def test_move_usable_predicate_trusts_showdown_over_stale_pp():
    from v_dance.play.vgc_base import _move_usable_predicate
    locked = _FakeMove("closecombat", pp=0)         # poke-env reports a STALE 0-PP
    other = _FakeMove("darkpulse", pp=16)

    class _B:
        available_moves = [[], [locked]]            # Showdown: slot 1's locked move IS usable

    pred, have = _move_usable_predicate(_B(), 1)
    assert have is True
    assert pred(locked) is True                     # usable per Showdown despite current_pp==0
    assert pred(other) is False                     # Choice-locked OUT (not in Showdown's list)


def test_move_usable_predicate_matches_normalized_id():
    from v_dance.play.vgc_base import _move_usable_predicate
    avail = _FakeMove("u-turn")                      # available_moves: cosmetically different id

    class _B:
        available_moves = [[avail]]

    pred, _ = _move_usable_predicate(_B(), 0)
    assert pred(_FakeMove("uturn")) is True          # mon.moves entry matched by NORMALISED id


def test_move_usable_predicate_no_list_falls_back_to_pp():
    from v_dance.play.vgc_base import _move_usable_predicate

    class _B:
        available_moves = [[]]                        # no authoritative list (request-less)

    pred, have = _move_usable_predicate(_B(), 0)
    assert have is False
    assert pred(_FakeMove("x", pp=5)) is True and pred(_FakeMove("x", pp=0)) is False


def test_choice_locked_move_stays_representable_despite_stale_pp(monkeypatch):
    """Regression for the Choice-Scarf collapse: the locked move IS in mon.moves but poke-env
    reported current_pp=0 for it while Showdown's available_moves[slot] lists it usable. The old
    mask dropped it (pp==0) -> only the colliding bench switch remained -> /choose default + the
    step was DISCARDED. Now the move stays legal and _safe_order builds it (model-driven)."""
    from poke_env.battle import Move
    from v_dance.play.vgc_base import (VGCPlayerBase, build_legal_action_mask, action_to_order,
                                       _switch_order_target)
    locked = Move("closecombat", gen=9)
    locked._current_pp = 0                           # stale-0 PP in mon.moves

    battle, bench0 = _make_collision_battle()
    _patch_single_bench(monkeypatch, bench0)
    battle.active_pokemon[1]._mv = {"closecombat": locked}   # slot 1 = the Choice-locked mon
    battle.available_moves[1] = [locked]                     # Showdown: usable this turn

    mask1 = build_legal_action_mask(battle, 1)
    legal = [i for i, ok in enumerate(mask1) if ok]
    assert 0 in legal and legal != [12]              # the locked MOVE stays legal (not switch-only)

    order_s0 = action_to_order(12, battle, 0)        # slot 0 takes the single bench switch
    taken = {_switch_order_target(order_s0)}
    order_s1 = VGCPlayerBase._safe_order(None, 12, battle, 1, taken_switch_targets=taken)
    assert order_s1 is not None                      # builds the locked move, NOT None/default
    assert "closecombat" in order_s1.message.lower()
