...
from v_dance.encoders.live_state_encoder import LiveStateEncoder as StateEncoder  # noqa: E402
...
def _find(name: str) -> Path:
    ...
    if hit is None:   # vods are local-only (not in the published repo) — skip on CI
        pytest.skip(f"replay fixture not present (local-only): {name}")
    return hit
...
def test_ally_switch_both_opp_mons_active_after_replacement(ally_switch_log):
    ...
    parser = __import__("v_dance.parser.vod_parser.replay_parser", fromlist=["ShowdownReplayParser"])
...
def test_gap6_opp_snapshot_prefix_reproduces_full_parse_on_clean_replay(clean_log):
    ...
    from v_dance.encoders.live_state_encoder import opp_snapshot_from_log_prefix
...
def test_live_mask_excludes_disabled_moves(clean_log):
    ...
    _repo_root_on_path()
    from v_dance.play.vgc_base import build_legal_action_mask, action_to_order
    from v_dance.encoders.state_encoder import NUM_MOVES
...
def test_live_mask_allows_ally_target_for_damaging_moves():
    ...
    from v_dance.play.vgc_base import build_legal_action_mask, action_to_order, _TARGET_OPP0, _TARGET_ALLY
    from v_dance.encoders.state_encoder import build_action_mask
...
def test_codec_attaches_ally_target_for_adjacent_ally_move():
    ...
    _repo_root_on_path()
    from poke_env.battle import Move
    from v_dance.play.vgc_base import action_to_order, _TARGET_ALLY
...
def test_codec_targets_first_opp_slot_when_pokeenv_sees_no_foe():
    ...
    _repo_root_on_path()
    from poke_env.battle import Move
    from v_dance.play.vgc_base import action_to_order, _TARGET_OPP0
...
@pytest.mark.parametrize("perspective", ["p1", "p2"])
def test_side_conditions_parity(dual_log, perspective):
    """Side conditions: the two global side-condition blocks must match live-vs-offline every turn,
    and every value stays in [0, 1].  Most channels are a binary presence bit, but v11 C.1 layered
    ENTRY HAZARDS (Spikes / Toxic Spikes) carry a FRACTIONAL normalized layer count — so this block is
    NOT strictly binary (the real invariant is offline==live parity, asserted below)."""
    username = H.player_usernames(dual_log)[perspective]
    live = H.live_vectors_per_turn(dual_log, username)
    offline = H.offline_vectors_per_turn(dual_log, perspective)
    active_seen = 0
    for turn in sorted(offline):
        ...
        for lo, hi in (H.OUR_SIDE_SC, H.OPP_SIDE_SC):
            ...
            assert (lv >= 0.0).all() and (lv <= 1.0).all(), f"live SC out of [0,1]: {lv}"
            assert (ov >= 0.0).all() and (ov <= 1.0).all(), f"offline SC out of [0,1]: {ov}"
            assert np.array_equal(lv, ov), (
...
def test_live_sc_emit_restricted_to_offline_tracked_channels():
    """The live encoder must only emit the side-condition channels the offline parser tracks: Tailwind +
    Reflect/Light Screen/Aurora Veil + (v11 C.1) the 4 entry hazards. The parser still does NOT track
    Safeguard/Mist/Lucky Chant, so the corpus holds them constant 0.0; emitting them live (on the
    un-spliced SC globals) would feed the net an out-of-distribution 1.0."""
    from v_dance.encoders.live_state_encoder import LiveStateEncoder
    from v_dance.encoders.state_encoder import _SC_IDX
    enc = LiveStateEncoder()
    if not hasattr(enc, "_offline_sc_idx"):
        import pytest
        pytest.skip("poke-env not importable — live SC index set not built")
    allowed = enc._offline_sc_idx
    for n in ("TAILWIND", "REFLECT", "LIGHT_SCREEN", "AURORA_VEIL",
              "SPIKES", "STEALTH_ROCK", "STICKY_WEB", "TOXIC_SPIKES",      # v11 C.1: hazards
              "SAFEGUARD", "MIST", "LUCKY_CHANT"):                         # v11 C.2b: persistent protections
        assert _SC_IDX[n] in allowed, f"{n} should be live-emitted (offline tracks it)"
    # Single-turn side-protects (Mat/Quick/Wide Guard, Crafty Shield), the G-Max conditions, and the
    # pledges are STILL un-tracked offline (single-turn / not in the VGC format) → must NOT be emitted live.
    for n in ("MAT_BLOCK", "QUICK_GUARD", "WIDE_GUARD", "CRAFTY_SHIELD", "G_MAX_STEELSURGE"):
        if n in _SC_IDX:
            assert _SC_IDX[n] not in allowed, f"{n} must NOT be live-emitted (offline never sets it)"
