...
def test_resolve_train_pool_all_expands_to_full_pool():
    ...
    # format-aware (the fix): an M-A run draws ONLY M-A teams — M-B mons/items are illegal in M-A,
    # so the old unfiltered pool got those teams server-rejected. (Format-EXPLICIT so the test is
    # immune to whatever active format an earlier test left set.)
