"""Tests for the gimmick codec in state_encoder.py.

The gimmick head is a SEPARATE per-slot 3-way decision {0=none, 1=mega, 2=tera} (v11 Phase D),
orthogonal to the frozen 16-way move/switch head.  These tests cover:
  2a constants + accessor (ACTION_DIM stays frozen 16; GIMMICK_DIM = 3)
  2b action_to_gimmick (mega/tera/none, mega>tera precedence)
  2c build_gimmick_mask (mega-capability + team-megaed + team-teraed + empty/fainted)
  2d annotate_transition_actions stamps gimmick_index + gimmick_mask, invariant

⚠ Tera is mod-disabled in Champions (canTerastallize → null) so the tera column is dormant in production
(team_teraed is always False on real data); these tests exercise the forward-compat plumbing directly.
"""
...
def test_gimmick_constants():
    ...
    assert se.GIMMICK_TERA == 2          # v11 Phase D
    assert se.GIMMICK_DIM == 3
    assert se.get_gimmick_dim() == 3
...
def test_action_to_gimmick_mega_move():
    assert se.action_to_gimmick({"action": "move", "move": "Body Press", "mega": True}) == se.GIMMICK_MEGA


def test_action_to_gimmick_tera_move():
    assert se.action_to_gimmick({"action": "move", "move": "Body Press", "tera": True}) == se.GIMMICK_TERA
    # mega takes precedence if both are somehow stamped (cannot happen on one real move)
    assert se.action_to_gimmick({"action": "move", "mega": True, "tera": True}) == se.GIMMICK_MEGA
...
def test_action_to_gimmick_switch_never_gimmicks():
    assert se.action_to_gimmick({"action": "switch", "species": "Incineroar"}) == 0


# ── 2c: build_gimmick_mask ──────────────────────────────────────────────────
def _mon(species, base=None, is_mega=False, is_fainted=False, is_terastallized=False):
    return {"species": species, "base_species": base or species,
            "is_mega": is_mega, "is_fainted": is_fainted, "is_terastallized": is_terastallized}
...
def test_gimmick_mask_capable_mon_allows_mega_and_tera():
    snap = {"our_active": {"our_a": _mon("Venusaur")}}
    m = se.build_gimmick_mask(snap)
    assert m["our_a"] == [1, 1, 1]       # none + mega + tera all legal
    assert m["our_b"] == [0, 0, 0]       # empty slot → all-zero


def test_gimmick_mask_noncapable_mon_forbids_mega_but_allows_tera():
    snap = {"our_active": {"our_a": _mon("Incineroar")}}
    m = se.build_gimmick_mask(snap)
    assert m["our_a"] == [1, 0, 1]       # none + tera legal, mega not (no mega forme)


def test_gimmick_mask_fainted_slot_all_zero():
    snap = {"our_active": {"our_a": _mon("Venusaur", is_fainted=True)}}
    assert se.build_gimmick_mask(snap)["our_a"] == [0, 0, 0]


def test_gimmick_mask_team_already_megaed_forbids_mega_but_tera_stays():
    # our_b already mega'd → our_a may no longer mega (one mega/team/game) — but tera is INDEPENDENT
    # and still legal for both (mega-spent != tera-spent).
    snap = {"our_active": {"our_a": _mon("Charizard"),
                           "our_b": _mon("Gengar-Mega", base="Gengar", is_mega=True)}}
    m = se.build_gimmick_mask(snap)
    assert m["our_a"] == [1, 0, 1]
    assert m["our_b"] == [1, 0, 1]


def test_gimmick_mask_team_already_teraed_forbids_tera_but_mega_stays():
    # our_b already tera'd → tera no longer legal for either slot; mega stays legal for the capable one.
    snap = {"our_active": {"our_a": _mon("Charizard"),
                           "our_b": _mon("Incineroar", is_terastallized=True)}}
    m = se.build_gimmick_mask(snap)
    assert m["our_a"] == [1, 1, 0]       # mega legal (capable, unspent), tera spent
    assert m["our_b"] == [1, 0, 0]       # Incineroar non-capable; tera spent


def test_gimmick_mask_team_megaed_detected_on_bench():
    snap = {"our_active": {"our_a": _mon("Charizard")},
            "our_bench": [_mon("Venusaur-Mega", base="Venusaur", is_mega=True)]}
    assert se.build_gimmick_mask(snap)["our_a"] == [1, 0, 1]   # mega spent, tera stays


def test_gimmick_mask_teraed_detected_on_bench():
    snap = {"our_active": {"our_a": _mon("Charizard")},
            "our_bench": [_mon("Incineroar", is_terastallized=True)]}
    assert se.build_gimmick_mask(snap)["our_a"] == [1, 1, 0]   # tera spent (bench), mega stays
...
def test_gimmick_mask_two_capable_mons_both_allow_mega_before_use():
    ...
    m = se.build_gimmick_mask(snap)
    assert m["our_a"] == [1, 1, 1]
    assert m["our_b"] == [1, 1, 1]
...
def test_annotate_stamps_gimmick_index_and_mask():
    ...
    assert t["gimmick_mask"]["our_a"] == [1, 1, 1]
    assert t["gimmick_mask"]["our_b"] == [1, 0, 1]                     # Incineroar: tera legal, mega not


def test_annotate_stamps_tera_gimmick_index():
    t = {"state_before_actions": _g_snap(), "our_actions": [
        {"slot": "our_b", "action": "move", "move": "Knock Off", "target_slot": "opp_a", "tera": True},
    ]}
    se.annotate_transition_actions(t)
    assert t["our_actions"][0]["gimmick_index"] == se.GIMMICK_TERA    # tera is mask-legal for any mon


def test_annotate_drops_gimmick_when_mon_not_mega_capable():
    # A mega flag on a non-capable mon (Incineroar): the gimmick mask forbids mega there, so the invariant
    # drops gimmick_index to None — never a wrong label.  (Does not occur on real data.)
    t = {"state_before_actions": _g_snap(), "our_actions": [
