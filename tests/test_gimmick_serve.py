...
def test_team_has_megaed_live():
    _repo_on_path()
    from poke_env.battle import Pokemon
    from v_dance.encoders.live_state_encoder import team_has_megaed_live
...
def test_encoder_method_still_delegates():
    ...
    _repo_on_path()
    from poke_env.battle import Pokemon
    from v_dance.encoders.state_encoder import StateEncoder  # alias of VodStateEncoder — no _is_mega
...
def test_gimmick_legal_mask_capable_and_noncapable():
    _repo_on_path()
    from v_dance.play.vgc_base import build_gimmick_legal_mask
    battle = _DBattle([_DMon("charizard"), _DMon("incineroar")])
    assert build_gimmick_legal_mask(battle, 0) == [True, True, True]    # capable: none+mega+tera
    assert build_gimmick_legal_mask(battle, 1) == [True, False, True]   # not mega-capable, tera still legal
...
def test_gimmick_legal_mask_fainted_and_empty():
    ...
    assert build_gimmick_legal_mask(battle, 0) == [False, False, False]  # fainted
    assert build_gimmick_legal_mask(battle, 1) == [False, False, False]  # empty slot
...
def test_gimmick_legal_mask_team_already_megaed():
    ...
    # charizard is capable, but the team already used its mega → mega forbidden; tera stays legal (independent).
    assert build_gimmick_legal_mask(battle, 0) == [True, False, True]
...
def test_action_to_order_megas_move_when_legal():
    _repo_on_path()
    from v_dance.play.vgc_base import action_to_order, GIMMICK_MEGA
...
def test_action_to_order_no_mega_when_gimmick_none():
    _repo_on_path()
    from v_dance.play.vgc_base import action_to_order, GIMMICK_NONE
...
def test_live_can_mega_handles_shapes():
    _repo_on_path()
    from v_dance.play.vgc_base import _live_can_mega
...
# ── v11 Phase D: tera serve (mirror of the mega gating) ──────────────────────
def test_action_to_order_teras_move_when_legal():
    _repo_on_path()
    from v_dance.play.vgc_base import action_to_order, GIMMICK_TERA
    b = _move_battle(can_mega=False)
    b.can_tera = [True, False]                    # server offers tera on slot 0
    order = action_to_order(0, b, 0, gimmick=GIMMICK_TERA)
    assert order is not None
    assert "terastallize" in order.message, f"expected a tera order: {order.message!r}"


def test_action_to_order_no_tera_when_can_tera_missing():
    # the PRODUCTION case: tera is mod-disabled so battle has no can_tera / it's falsy → plain move
    _repo_on_path()
    from v_dance.play.vgc_base import action_to_order, GIMMICK_TERA
    order = action_to_order(0, _move_battle(can_mega=True), 0, gimmick=GIMMICK_TERA)
    assert order is not None
    assert "terastallize" not in order.message and "mega" not in order.message


def test_live_can_tera_handles_shapes():
    _repo_on_path()
    from v_dance.play.vgc_base import _live_can_tera
    ...
    class _Missing:     # mod-disabled / older poke-env
        pass

    assert _live_can_tera(_L(), 0) is True
    assert _live_can_tera(_L(), 1) is False
    assert _live_can_tera(_L(), 5) is False          # out of range → False
    assert _live_can_tera(_B(), 0) is True
    assert _live_can_tera(_Missing(), 0) is False     # no attr → False
...
def test_safe_order_threads_gimmick_to_primary_action():
    ...
    _repo_on_path()
    from v_dance.play.vgc_base import VGCPlayerBase, GIMMICK_MEGA, GIMMICK_NONE
