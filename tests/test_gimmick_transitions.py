...
def test_turn_transition_mega_move_has_gimmick_index_and_mask(tmp_path):
    ...
    # gimmick mask (v11 Phase D, 3-wide): Venusaur mega-capable -> [1,1,1]; Incineroar -> [1,0,1] (tera legal)
    assert t["gimmick_mask"]["our_a"] == [1, 1, 1]
    assert t["gimmick_mask"]["our_b"] == [1, 0, 1]
...
def test_turn_gimmick_mask_present_on_every_turn_transition(tmp_path):
    for t in _run(tmp_path, _MEGA_LOG):
        if t["decision_type"] == "turn":
            ...
            assert all(len(r) == 3 for r in t["gimmick_mask"].values())   # v11 Phase D: 3-wide
...
def test_replacement_transition_gimmick_is_switch_only(tmp_path):
    ...
    # deciding (fainted) slot our_a: only "none" legal (a replacement never gimmicks); ally our_b not deciding.
    assert repl["gimmick_mask"]["our_a"] == [1, 0, 0]
    assert repl["gimmick_mask"]["our_b"] == [0, 0, 0]
