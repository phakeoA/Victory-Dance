...
from v_dance.datatools.policy_analysis import (  # noqa: E402
...
def test_move_block_offset_matches_encoder():
    ...
    assert MOVE_BLOCK_START == 71  # v9 layout (moves start after the per-mon weight feature)
    enc = VodStateEncoder()
...
def test_flatten_move_known():
    from v_dance.encoders.state_encoder import MOVE_FEATURES
    vec = np.zeros(POKEMON_FEATURES * 2, dtype=np.float32)
    base = move_block_start(0)
