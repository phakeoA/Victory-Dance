...
def test_real_hp_faint_delta_uses_stored_max_not_100():
    """A KO line carries NO denominator (``|-damage|MON|0 fnt``); on a real-HP replay it must NOT scale the
    delta by a bogus 100 — the KO's hp_pct_delta must use the mon's stored real max (gap #5 / audit 2026-06-30)."""
    r = parse(
        "|switch|p1a: Dondozo|Dondozo, L50, M|200/200",
        "|switch|p2a: Aerodactyl|Aerodactyl, L50, M|100/100",
        "|turn|1",
        "|move|p2a: Aerodactyl|Rock Slide|p1a: Dondozo",
        "|-damage|p1a: Dondozo|100/200",       # chipped to 50%
        "|turn|2",
        "|move|p2a: Aerodactyl|Rock Slide|p1a: Dondozo",
        "|-damage|p1a: Dondozo|0 fnt",         # KO — denominator-less
        "|faint|p1a: Dondozo",
        "|turn|3",
        "|win|bob",
    )
    ko = [e for t in r["turns"] for e in t.get("damage_events", [])
          if e.get("species") == "Dondozo" and e.get("hp_pct_after") == 0.0]
    assert ko, "KO damage event not found"
    # 50% -> 0% over the real 200 max is -50%, NOT the -100 a clobbered /100 denominator would give.
    assert ko[0]["hp_pct_delta"] == -50.0
