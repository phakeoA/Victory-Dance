...
def _ac(tmp_path):
    return ActorCritic.from_bc_checkpoint(write_attn_ckpt(tmp_path / "bc.pt"))
...
def test_record_decision_captures_logprob_value_masks(tmp_path):
    ...
    G.record_decision(c, ac, state=_state(1), a0=3, a1=7, g0=0, g1=0,
                      mask0=[1] * AD, mask1=[1] * AD, gmask0=[1, 0, 0], gmask1=[1, 0, 0],
    ...
    assert t.mask_s0 == [1] * AD and t.gmask_s0 == [1, 0, 0]
...
def test_record_decision_tau_zero_finite_logprob(tmp_path):
    """Scan 2026-07-02 (MED): the --sample ab argmax seat records at τ=0 —
    logits/τ would NaN the log-softmax and be silently stored. The argmax action
    has probability 1 in the τ→0 limit → logprob 0.0; value stays finite."""
    ac = _ac(tmp_path)
    c = TrajectoryCollector("g", "p1")
    G.record_decision(c, ac, state=_state(3), a0=1, a1=2, g0=0, g1=0,
                      mask0=[1] * AD, mask1=[1] * AD, gmask0=[1, 0, 0], gmask1=[1, 0, 0],
                      decision_type="turn", turn=1, tau=0.0)
    t = c.finish(own_team=["a"] * 6, opp_team=["b"] * 6, tp_bring=[0], tp_leads=[0],
                 won=True, terminal_type="win").transitions[0]
    assert np.isfinite(t.logprob) and t.logprob == 0.0
    assert np.isfinite(t.value) and -1.0 <= t.value <= 1.0
...
def test_finalize_stamps_sampling_meta():
    ...
    traj = G.finalize_trajectory(c, won=True, terminal_type="win", own_team=["a"] * 6,
                                 n_turns=3, sampling={"tau": 0.45, "top_p": 0.9})
    assert traj.meta.sampling == {"tau": 0.45, "top_p": 0.9}
    # serialization round-trip keeps it; legacy dicts without the key stay loadable
    assert EpisodeMeta.from_obj(traj.meta.to_obj()).sampling == {"tau": 0.45, "top_p": 0.9}
    legacy = traj.meta.to_obj()
    legacy.pop("sampling")
    assert EpisodeMeta.from_obj(legacy).sampling is None
...
def test_phase0_finalize_failed_surfaced_but_not_counted():
    # a finalize failure is post-game BOOKKEEPING (the game is dropped from the sample), NOT a non-model
    # turn execution — so it is surfaced in the report yet excluded from the MODEL-DRIVEN denominator.
    rep = G.phase0_report(_games([True, False] * 3), {"finalize_failed": 5}, min_games=4)
    assert rep["finalize_failed"] == 5
    assert rep["non_model_executed"] == 0
    assert rep["model_driven"] == 1.0 and rep["PASS"]
...
def test_run_self_play_games_runs_all_pairings_via_shared_runner(monkeypatch):
    ...
    # ⚠ run_self_play_games(manage_server=True) starts servers via ServerPool, whose default
    # start_fn is start_showdown_on — NOT the start_showdown wrapper patched above. Unpatched,
    # this test spawned a REAL `node pokemon-showdown` on port 8000 and the no-op stop_showdown
    # patch then orphaned it: every full-suite run leaked a live server cluster (found 2026-07-10
    # via a Popen-spy pytest plugin; the leak hid whenever a previous leak already held the port).
    monkeypatch.setattr(R, "start_showdown_on", lambda port=8000: None)
