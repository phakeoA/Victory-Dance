...
def test_admit_and_record_result():
    """With decay OFF (pfsp_decay=1.0) record_result is the old exact running count."""
    lg = OpponentLeague(latest_path="latest.pt", cfg=LeagueConfig(pfsp_decay=1.0))
    lg.admit("g1", "g1.pt", 1)
...
# ── #24 PFSP EMA decay (tallies track the CURRENT latest across a HOLD) ────────
def test_record_result_ema_tracks_current_policy():
    """#24: with the default EMA decay, old outcomes age out so the win-rate reflects the
    RECENT (current-policy) games. A snapshot the latest used to LOSE to but now BEATS
    converges toward a high win-rate instead of being dragged to ~0.5 by the stale history."""
    lg = OpponentLeague(latest_path="x", cfg=LeagueConfig(pfsp_decay=0.9))
    lg.admit("g1", "g1.pt", 1)
    for _ in range(50):                       # weak ancestor policy: all losses
        lg.record_result("g1", latest_won=False)
    for _ in range(50):                       # current strong policy: all wins
        lg.record_result("g1", latest_won=True)
    s = lg.snapshots[0]
    assert s.latest_winrate() > 0.95          # the recent wins dominate (not dragged to 0.5)
    assert s.games_vs_latest < 11             # bounded EMA accumulator ≈ 1/(1-0.9)=10, not 100


def test_record_result_ema_persists_as_float():
    """The EMA tallies are floats and survive the resume round-trip (was int-cast before #24)."""
    lg = _league()
    lg.admit("g1", "g1.pt", 1)
    lg.record_result("g1", latest_won=True)
    lg.record_result("g1", latest_won=False)
    back = OpponentLeague.from_obj(lg.to_obj())
    s0, s1 = lg.snapshots[0], back.snapshots[0]
    assert isinstance(s1.games_vs_latest, float)
    assert s1.games_vs_latest == pytest.approx(s0.games_vs_latest)
    assert s1.wins_vs_latest == pytest.approx(s0.wins_vs_latest)
    assert back.cfg.pfsp_decay == pytest.approx(lg.cfg.pfsp_decay)
