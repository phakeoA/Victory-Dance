...
def _write_ckpt(path):
    return write_attn_ckpt(path, seed=2)
...
def _onpolicy_txns(ac, n=6):
    ...
    txns = [_txn(i % ACTION_DIM, PASS_ACTION, g0=0, m0=[1] * ACTION_DIM, gm0=[1, 0, 0], seed=i)
            for i in range(n)]
...
def test_from_batch_backprops_to_actor_and_critic(tmp_path):
    ...
    # opp_ce is a NaN sentinel here (this ckpt has no opp_a/opp_b heads -> aux CE n/a),
    # like kl_to_bc would be without a ref; every actually-computed stat must be finite.
    assert math.isnan(st["opp_ce"])
    assert all(math.isfinite(v) for k, v in st.items() if k != "opp_ce")
