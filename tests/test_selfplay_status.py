...
# ── fs-monitor: per-gen edge tally + run-wide accumulation -> status.json ──────
def test_set_gen_counts_writes_per_gen_and_accumulates(tmp_path):
    ls = _ls(tmp_path)
    ls.start_run(n_generations=3)
    ls.set_gen_counts({"forced_default": 2, "forfeit": 1, "total": 3})
    s = read_status(ls.path)["run"]
    assert s["fs_monitor"] == {"forced_default": 2, "forfeit": 1, "total": 3}
    assert s["fs_monitor_total"] == {"forced_default": 2, "forfeit": 1, "total": 3}
    # a second generation REPLACES fs_monitor (per-gen) but ADDS into fs_monitor_total (cumulative)
    ls.set_gen_counts({"forced_default": 1, "forfeit": 0, "total": 1})
    s = read_status(ls.path)["run"]
    assert s["fs_monitor"] == {"forced_default": 1, "forfeit": 0, "total": 1}
    assert s["fs_monitor_total"] == {"forced_default": 3, "forfeit": 1, "total": 4}


def test_set_gen_counts_coerces_to_int(tmp_path):
    ls = _ls(tmp_path)
    ls.start_run(n_generations=1)
    ls.set_gen_counts({"forced_default": 2.0})       # numpy/float in → plain int out (json-clean)
    assert read_status(ls.path)["run"]["fs_monitor"]["forced_default"] == 2
