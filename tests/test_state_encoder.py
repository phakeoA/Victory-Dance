...
from v_dance.encoders.state_encoder import (  # noqa: E402
    ...
    _MOVE_BLOCK_REL, ITEM_ID_REL, ABILITY_ID_REL, NUM_ITEM_TAGS, NUM_ABILITY_TAGS,  # v9
)
from v_dance.encoders.mechanic_tags import ITEM_TAG_NAMES, ABILITY_TAG_NAMES  # noqa: E402  (v9)

# v9 item/ability blocks within a mon = [identity index] + [tags] + [known]; tags start after the index.
_ITEM_TAGS0 = ITEM_ID_REL + 1
...
_MOVE0 = _MOVE_BLOCK_REL                                # first move block base (v9: 71, after weight)
_SPREAD_REL = 8                                         # is_spread (fixed; type-eff 9-12, is_known last)
...
def test_item_ability_block_written_in_snapshot():
    ...
    vec = StateEncoder().encode_snapshot(snap, turn=1)
    item_on = {ITEM_TAG_NAMES[k] for k in range(NUM_ITEM_TAGS) if vec[_ITEM_TAGS0 + k] == 1.0}
    assert item_on == {"has_item", "assault_vest"}
    assert vec[_ITEM_TAGS0 + NUM_ITEM_TAGS] == 1.0        # item_known
    abil_on = {ABILITY_TAG_NAMES[k] for k in range(NUM_ABILITY_TAGS) if vec[_ABIL_TAGS0 + k] == 1.0}
    assert abil_on == {"intimidate"}
    assert vec[_ABIL_TAGS0 + NUM_ABILITY_TAGS] == 1.0     # ability_known
