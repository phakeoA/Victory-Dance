...
def test_end_to_end_smoke_train(tmp_path):
    ...
    assert (out / "teampreview_base.pt").exists()
    ckpt = torch.load(out / "teampreview_base.pt", map_location="cpu", weights_only=False)
...
def test_smoke_train_with_type_a_and_patience(tmp_path):
    ...
    res = trainmod.train(args)
    assert (out / "teampreview_base.pt").exists()
    ck = torch.load(out / "teampreview_base.pt", map_location="cpu", weights_only=False)
...
def test_default_out_per_features_mode_matches_consumer_dirs():
    ...
    # explicit --out always wins, regardless of mode
    assert trainmod.parse_args(["--features", "sbda", "--out", "zzz"]).out == "zzz"
