...
def _build_eval_players_real(candidate, prev_best, team_chooser, spec: EvalSpec):
    ...
    global uid so they're unique across worker processes."""
...
def eval_worker(payload):
    ...
    candidate, prev_best, team_chooser, specs, battle_timeout, async_workers = payload
...
def eval_with_pool(candidate, *, opponents, team_pool, battles_per_opponent: int, team_chooser,
    ...
    payloads = [(str(candidate), pb, tc, batch, battle_timeout, async_per_proc) for batch in batches]
